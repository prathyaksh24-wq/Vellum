...
describe("Vellum App Action API adapter", () => {
  ...
  test("uses the catalog, dispatch, and Undo endpoints", async () => {
    ...
    expect(request.mock.calls[2][0]).toBe("/api/app-actions/undo");
    expect(JSON.parse(request.mock.calls[2][1].body)).toEqual({ token: "undo-1", context });
  });
});
