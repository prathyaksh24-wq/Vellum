...
from agent.mcp import apify_tools, firecrawl_tools, github_tools, obsidian_tools, playwright_tools, tavily_tools
...
def test_multiple_mcp_servers_can_run_concurrently(monkeypatch):
    ...
    async def fake_tavily(params):
        events.append("tavily-start")
        await asyncio.sleep(0.02)
        events.append("tavily-end")
        return "tavily-ok"
    ...
    assert [item.result for item in results] == ["tavily-ok", "apify-ok", "browser-ok", "github-ok", "obsidian-ok"]
    assert events[:2] == ["tavily-start", "apify-start"]
