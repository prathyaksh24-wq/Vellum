...
> Paste this into Claude Code or Codex as the operational contract for every build session.
...
skill for Claude Code and Codex. It maps the Vellum codebase into a queryable
knowledge graph, making build sessions coherent across long conversations.

Install once:
```bash
pip install graphifyy
graphify install   # registers /graphify for Claude Code
```

Use at the start of every build session in Claude Code:
```
/graphify .
```
