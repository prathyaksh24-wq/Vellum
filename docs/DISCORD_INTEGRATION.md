...
DISCORD_AUTONOMOUS_CHANNEL_IDS=...
```
...
- Sending requires explicit confirmation unless the channel is also in
  `DISCORD_AUTONOMOUS_CHANNEL_IDS`.
- Every autonomous channel must also be present in the ordinary channel
  allowlist.
...
The URL requests only View Channels, Read Message History, and Send Messages.
Do not grant Administrator, Manage Messages, or unrelated permissions.

The initial connector supports bot identity, installed server discovery,
allowlisted channel discovery, recent message reads, and text messages. Discord
data-package ingestion is intentionally separate and will be implemented against
a real export when one is available.
